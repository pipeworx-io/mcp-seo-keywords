...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'SEO Keywords');
}
...
// Wave-2 CostModel (§4). seo-keywords (Labs keyword_overview) ≈ $0.0101 (1 kw),
// roughly flat per-task → ~160 credits est at the 10-kw cap. MEASURE at 10
// keywords and set base = ceil(cost×15000) before PLATFORM_DATAFORSEO_AUTH goes
// live. Only charged when our platform key backs the call (BYO is free).
export default { tools, callTool, cost: { base: 198 }, provider: 'dataforseo' } satisfies McpToolExport;
